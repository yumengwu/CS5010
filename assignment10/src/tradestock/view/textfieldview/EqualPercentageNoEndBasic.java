 ...
 * same weight on every stock in a strategy. It enables use to input the number of stocks in
 * the strategy, the name of the strategy, start date, interval of the strategy and how much money
 * the user invest every period.
 */
public class EqualPercentageNoEndBasic extends JFrame implements WithTextField {
  ...
  /**
   * Constructor of EqualPercentageNoEndBasic, it initialize the view including label, text fields
   * combobox and buttons.
   * @param caption caption.
   */
  public EqualPercentageNoEndBasic(String caption) {
    super(caption);
    JLabel label;
    ...
    next.setActionCommand("equal percentage no end basic next");
    home.setActionCommand("equal percentage no end basic home");
    hint = new JLabel("");
    panel.add(label);
    panel.add(number);
    hintPanel.add(hint);
